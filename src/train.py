...
class model(object):
    ...
    def show(self):
        print(self.config.get('data', 'training_set'))

    def get_training_embedding(self):
        preprocess.load_qa_list([self.config.get('data', 'training_set')])
    ...
    def model(self):
        ...
        print('input_q_shape:'+ str(input_q.shape))
        # 对输入层进行编码
    ...
    def train(self):
        ...
        epoches = self.config.get('training_rule', 'epoches')
        batch_size = self.config.get('training_rule', 'batch_size')
        model_save_path = self.config.get('training_rule', 'model_path')
        word_dict_save_path = self.config.get('training_rule', 'word_dict_path')
        batch_per_epoch = len(preprocess.qa_list) // batch_size * 5
